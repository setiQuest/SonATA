...
void NssParameters::printMiscCommandsHelp(ostream &os) const
{
   ...
   os << "    exec <command> \t\t\texecute an operating system command\n"
      ...
      << "    expectedcomponents \t\t\tdisplay the 'expected NSS components' \n\t\t\t\t\tconfig file info\n"
      << "    followup <prev act id> \t\tstart a followup of a previous activity\n"
      << "    freeants \t\t\t\tshorthand for 'start freeants'\n"
      << "    grid <prev act id> \t\t\tstart a grid pointing followup of a \n\t\t\t\t\tprevious activity\n"
      ...
      << "    para default \t\t\tset all parameters to their \n\t\t\t\t\tdefault values\n"
      ...
      << "    sh <command> \t\t\tuse the unix shell to execute an \n\t\t\t\t\toperating system command\n"
      << "    show \t\t\t\tshow all parameter settings\n"
      << "    start <'tasks' | <task>[,<task>...]>\n\t\t\t\t\tstart selected tasks (i.e., strategies)\n"
      << "    status \t\t\t\tdisplay status of all components\n"
      << "    stop \t\t\t\tstop all activities\n"
      << "    wrapup \t\t\t\tcomplete currently running activities \n\t\t\t\t\t& any followups, then stop\n"
      << "    targetidallbeams <id> \t\tset the target id on all beams in i\n\t\t\t\t\tact params\n"
