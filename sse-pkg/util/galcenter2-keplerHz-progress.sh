...
EOF
mysql -t galcenter2 << EOF
select  count(distinct targetId ) as CompletedGridPoints from ActivityUnits, Activities where ActivityUnits.activityId = Activities.id and dxHighFreqMhz  > 6966.0 and Activities.type = 'target' and ActivityUnits.validObservation = 'Yes' and targetId > 31999 and targetId < 33000;
EOF
echo ""
