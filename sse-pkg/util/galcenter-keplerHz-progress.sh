...
EOF
mysql -t galcenter << EOF
select  count(distinct activityId, targetId) as CompletedGridPoints from ActivityUnits, Activities where ActivityUnits.activityId = Activities.id and dxHighFreqMhz  > 8919.3 and Activities.type = 'target' and ActivityUnits.validObservation = 'Yes' and targetId > 31999 and targetId < 33000;
EOF
echo ""
